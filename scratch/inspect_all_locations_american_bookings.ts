...
async function main() {
  ...
  console.log("Fetching all American Pool Table tables...");
  const { data: tables } = await supabase
    .from("tables")
    .select("id, name, location_id, location:locations(name)")
    .eq("name", "American Pool Table");

  console.log("Tables:", JSON.stringify(tables, null, 2));

  console.log("\nFetching all bookings for these tables today...");
  const { data: bookings } = await supabase
    .from("bookings")
    .select(`
      id, status, scheduled_start, scheduled_end,
      order_item:order_items!inner(table_id, table:tables(name, location:locations(name))),
      order:orders(customer_name, customer_phone, status)
    `)
    .in("status", ["confirmed", "checked_in"])
    .in("order_items.table_id", tables?.map(t => t.id) ?? []);
